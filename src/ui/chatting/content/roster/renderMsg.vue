  ...
  <div>
    ...
    <div v-else :class="{ messageFrame: true, self: isSelf, roster: !isSelf }">
      ...
      <div :class="{ multiForwardRoster: !isSelf && showMultiForward, 'message-inner-group': true }">
        ...
        <div class="contentFrame message-content-wrapper">
          ...
          <div :class="{ c_content_more: true, 'message-action-footer': true, 'is-menu-open': showActionMenu }" v-if="!showMultiForward">
            ...
            <div :class="{ 'action-icon-wrapper': true, self: isSelf, roster: !isSelf, 'message-footer-controls': true }">
              ...
              <div :class="{ messageExt: true, 'message-menu': true, 'message-menu-self': isSelf, 'message-menu-other': !isSelf, 'is-visible': showActionMenu }" @click.stop>
                <div @click="handleDeleteMessage" class="item delete">{{ $t('删除') }}</div>
                <div @click="handleRecallMessage" class="recall item" v-if="isSelf">{{ $t('撤回') }}</div>

                <div @click="handleUnreadMessage" class="set_unread item" v-if="messageStatus !== 'unread' && !isSelf">{{ $t('设置未读') }}</div>
              </div>
            </div>
            <span class="message-footer-status" v-if="messageStatusMeta">
              <span :class="['message-footer-status__icon', `is-${messageStatusMeta.key}`]" aria-hidden="true">
                <svg v-if="messageStatusMeta.key === 'unread'" viewBox="0 0 24 24">
                  <circle cx="12" cy="12" r="6"></circle>
                </svg>
                <svg v-else-if="messageStatusMeta.key === 'delivered'" viewBox="0 0 24 24">
                  <path d="M6.5 12.5l3 3 8-8"></path>
                </svg>
                <svg v-else viewBox="0 0 24 24">
                  <path d="M5.5 12.5l3 3 5-5"></path>
                  <path d="M11.5 12.5l3 3 5-5"></path>
                </svg>
              </span>
              <span>{{ messageStatusMeta.label }}</span>
            </span>
          </div>
        </div>
      </div>
...
export default {
  ...
  mounted() {
    ...
    if (shouldAutoRead && !this.getAutoReadSuppressed) {
  ...
  props: ['message'],
  computed: {
    ...mapGetters('content', ['getSid', 'getMessageTime', 'getAutoReadSuppressed']),
    ...
    messageStatusMeta() {
      ...
      if (!this.isSelf && status !== 'unread') {
        return null;
      }
      if (status === 'unread') {
        return { key: 'unread', label: this.$t('未读') };
      }
      if (status === 'delivered') {
        return { key: 'delivered', label: this.$t('送达') };
      }
      if (status === 'read') {
        return { key: 'read', label: this.$t('已读') };
      }
      return null;
    },
    isSendFailed() {
  ...
  methods: {
    ...
    handleUnreadMessage() {
      this.$store.dispatch('content/actionMarkCurrentMessageUnread', {
        mid: this.message.id
      });
      this.unreadMessage();
      this.closeActionMenu();
