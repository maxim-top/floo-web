      ...
      <div class="message-inner-group">
        <div class="rosterInfo">
          <el-popover placement="left-start" trigger="hover" width="170" :visible-arrow="false" :append-to-body="true">
            ...
            <div class="profile-bio" v-if="rosterProfile.nick_name">{{ $t('昵称：') }}{{ rosterProfile.nick_name }}</div>
            <div class="profile-bio">{{ $t('用户名：') }}{{ rosterProfile.username }}</div>
            <div class="profile-bio">{{ $t('ID：') }} {{ rosterProfile.user_id }}</div>
