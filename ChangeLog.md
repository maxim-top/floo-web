...
### 3.0.19 20260706
1.增加多语言支持，修复语言切换、语言刷新及相对时间本地化展示问题。
2.优化登录和注册体验，调整登录页响应式布局，修复切换账号、登录失败提示及AppID解析相关问题。
3.优化微信小程序与LinkChat唤起流程，修复首次启动、二维码登录、环境参数传递及跳转稳定性问题。
4.新增发起新会话能力，支持群创建成员搜索，并补充AppID历史记录等辅助功能。
5.修复流式消息展示、刷新、代码高亮及消息追加到错误会话等问题，提升消息展示稳定性。
6.优化系统消息展示逻辑，修复系统会话切换和会话列表最后一条消息显示异常问题。
7.修复联系人、群组及群成员列表中的默认头像、无效用户过滤、群退出文案及个人信息展示问题。
8.优化support页面与LinkChat界面，补充来源展示、徽标显示，并修复浮动按钮、最小化、背景和移动端交互样式问题。
9.持续优化聊天界面、联系人、转发流程及移动端整体UI，提升多端布局与交互体验。

### 3.0.18 20250925
1. 修复旧AppID状态异常时，LinkChat无法切换到新AppID的问题。
2. DNS支持更新link和dns服务器列表。
3. 修复会话搜索可能报错的问题。
4. 点击会话总未读数时，如果会话有未读，会滚动会话列表到未读会话的位置。
5. 存储会话打开状态，从其它界面回复到会话列表界面时，会恢复之前打开的会话。
6. WebP格式动图无法生成缩略图的问题。

### 3.0.17 20250826
1. 上传下载文件时，使用已测速的访问最快的域名。
2. 接口访问速度测试。
3. 修复第二次批量转发时状态异常的问题。
4. 存储不足时自动释放空间。
5. 切换会话后正在输入状态显示异常的问题。
6. 增加用户实名认证功能。
7. 批量转发时记录已转发的用户和群，转发列表全选时可以忽略已转发的用户和群。
8. Link页面掉线时显示最小化图标，点击后重连。
9. 从DNS服务器读取App认证信息。
10. 群开启已读功能后，会话列表可能错误创建的问题。
11. 拉取历史消息显示异常的问题。
12. 优先显示用户备注。
